...
when using a-repeat to populate a select or datalist element with options and the bound property on the model is a simple array ie. ['foo', 'bar'], use {{item}} as the variable in the template.
...
### Using a Custom Element as the model
...
<a-bind model="dom:#my-ce" property="name">
  <input type="text">
</a-bind>
```
...
-v3.0.0 : Refactored for performance and security
